...
class EventParams:
	def __init__(self, date=None, time=None):
		self.loan_id:			Union[int, None] = 0	#Fetch from DB
		self.event_id:		Union[int, None] = 0	#Assigned by DB
		self.event_date:	Optional[dt.date]= date or dt.now().date()
		self.event_time:	Optional[dt.timetz] = time or dt.now().timetz()
...
class PrincipalLending(EventParams):
	def __init__(self, principal_lending_currency_sum = None):
		super().__init__()
...
Lending_1 = PrincipalLending(principal_lending_currency_sum = 300000)
