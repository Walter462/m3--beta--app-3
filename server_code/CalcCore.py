...
def extract_sorted_events_properties() -> List[Dict[str, str]]:
    ...
    for event in events_list:
        event_dict = {
            ...
            "principal_lending_currency.currency_to_loan_rate": str(f"{event.principal_lending_currency.currency_to_loan_rate:.4f}") if event.principal_lending_currency else None,
            "principal_lending": str(f"{event.principal_lending:,.2f}") if event.principal_lending is not None else None,
            "interest_rate": f"{event.interest_rate:.2%}" if event.interest_rate is not None else None,
            "interest_rate_base": str(event.interest_rate_base) if event.interest_rate_base is not None else None,
            "principal_repayment_currency.currency_amount": str(f"{event.principal_repayment_currency.currency_amount:,.2f}") if event.principal_repayment_currency else None,
            ...
            "principal_repayment_currency.currency_to_loan_rate": str("{event.principal_repayment_currency.currency_to_loan_rate:.4f}") if event.principal_repayment_currency else None,
            "principal_repayment": str(f"{event.principal_repayment:,.2f}") if event.principal_repayment is not None else None,
            "interest_repayment_currency.currency_amount": str(f"{event.interest_repayment_currency.currency_amount:,.2f}") if event.interest_repayment_currency else None,
            ...
            "interest_repayment_currency.currency_to_loan_rate": str(f"{event.interest_repayment_currency.currency_to_loan_rate:.4f}") if event.interest_repayment_currency else None,
            "interest_repayment": str(f"{event.interest_repayment:,.2f}") if event.interest_repayment is not None else None,
        }
...
def extract_calculated_loan_properties() -> List[Dict[str, str]]:
    ...
    for event in calculated_loan:
        loan_dict = {
            ...
            "Pr_balance": str(f"{event.principal_balance:,.2f}") if event.principal_balance is not None else None,
            "Int_rate": f"{event.interest_rate:.2%}" if event.interest_rate is not None else None,
            "Int_base": str(event.interest_rate_base) if event.interest_rate_base is not None else None,
            "Int_accrued": str(f"{event.interest_accrued:.2f}") if event.interest_accrued is not None else None,
            "Int_rep": str(f"{event.interest_repayment:,.2f}") if event.interest_repayment is not None else None,
            "Int_balance": str(f"{event.interest_balance:,.2f}") if event.interest_balance is not None else None,
        }
